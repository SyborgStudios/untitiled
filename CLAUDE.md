...
- `audio/` — Audiofassungen von Werkstattdokumenten (per ElevenLabs-API erzeugt; ungeschwärzt, **nicht** für `site/` bestimmt; per `.gitignore` vom Repo ausgenommen). Der API-Key liegt in `.secrets/elevenlabs_api_key.txt` — eingeschränkter Key: nur Text-to-Speech erlaubt, `voices_read`/`user_read` gesperrt. Bisher genutzte Stimme: „Daniel" (`onwK4e9ZLuTAKqWW03F9`), Modell `eleven_multilingual_v2`; Texte >2.400 Zeichen an Absatzgrenzen chunken. Achtung: Das lokale Homebrew-ffmpeg ist defekt (libjxl-Version fehlt) — MP3-Teile stattdessen per Python zusammenfügen (ID3- und Xing/Info-Frames der Teile entfernen, Frames konkatenieren).
  ...
  - `audio/drehbuch-v1-lesefassung-de.mp3` — **Lesefassung, ein Sprecher** (angelegt 28.07.2026 auf Wunsch des Autors): Daniel liest das ganze Drehbuch vor, wie man ein Drehbuch vorliest. Gebaut mit `audio/make_lesefassung.py` (gleiche Modi `dry`/`gen`/`concat` `[de|en]`, gleiche MP3-Konkatenation ohne ffmpeg; Segmente in `audio/lesefassung/segments/`). Anders als das Hörspiel werden **mitgelesen**: Szenenüberschriften (vorlesbar normalisiert — „Innen. Speicher — Nacht."), Figurennamen vor dem Dialog („Anna: …"), `SCHWARZ.`, `HARTER SCHNITT`, zentrierte Blöcke, dazu ein kurzer Titelvorspann. **Nicht** mitgelesen: Synopsen (`=`), Werkstatt-Notes `[[…]]`, ANMERKUNG-Blöcke, Parentheticals, Beat-Metadaten. Die Vater-Setzung gilt auch hier (Treatment-Prosa statt des Worts unter der Tür). Bei `(CONT'D)` desselben Sprechers wird der Name nicht wiederholt. Weil nur eine Stimme spricht, bündelt das Skript den Text zu wenigen großen Segmenten (28 statt 193) — schneller und weniger API-Aufrufe. **Aktuelle Fassung: Stand 03.08.2026** (32 Segmente, ~70k Zeichen, ~82 Min) — enthält zusätzlich „Kisten vom Himmel" (gestraffte Fassung), den Waschhaus-Hund samt Abend-Miniszene und die Räumungs-Runde des Autors (Notes raus, 52 ohne Marker). Ältere Fassungen archiviert als `drehbuch-v1-lesefassung-de-20260728.mp3`, `-20260801.mp3` und `-20260802.mp3`. Vor jeder Neugenerierung `lesefassung/segments/` komplett leeren (Textänderungen verschieben alle Segmentnummern). Die EN-Konfiguration ist im Skript angelegt, aber noch nicht gebaut.
